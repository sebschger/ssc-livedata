...
DATA_MESSAGE_LIST_NAME = (
    "rx.metadata"  # Hier wird definiert, wo die Metadaten des Payloads liegen
)
...
def interpret_message():
    ...
    while True:
        ...
        try:
            ...
            print("Problem beim Parsen der Daten:")
            print(item)
        finally:
...
@listener.route(API_WEBHOOKNAME, methods=["POST"])
def handle_incoming():
...
def clean_data(item: dict):
    ...
    if DATA_MESSAGE_LIST_NAME in df_raw.columns:
        metadata = df_raw[DATA_MESSAGE_LIST_NAME].apply(lambda x: x[0] if x else [])

        df_metadata = pd.json_normalize(metadata)
        df_metadata.index = df_raw.index
        df_metadata.columns = DATA_MESSAGE_LIST_NAME + "." + df_metadata.columns

        df_cleaned = pd.concat(
            [df_raw.drop(columns=[DATA_MESSAGE_LIST_NAME]), df_metadata], axis=1
        )
...
def main():
    ...
    try:
        ...
        print(f"Problem mit dem Waitress-Prozess (Server):\n{e}")
    finally:
